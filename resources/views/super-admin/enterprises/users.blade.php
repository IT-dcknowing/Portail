...
function createUserRow(users) {
    ...
    // Créez une ligne pour chaque utilisateur
        ...
        tr.innerHTML = `
            <td>
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h6 class="mb-0">${user.name || 'Nom non spécifié'}</h6>
                        <small class="text-muted">${user.username || ''}</small>
                    </div>
                </div>
            </td>
            <td>${typeBadge}</td>
            <td>${user.email || 'Email non spécifié'}</td>
            <td>${user.phone || 'N/A'}</td>
            <td>${departments}</td>
            <td>${statusBadge}</td>
            <td>${user.last_login ? formatDate(user.last_login) : 'Jamais'}</td>
            <td><small>${formatDate(user.created_at) || 'Date non spécifiée'}</small></td>
            <td align="center">
                <div class="dropdown">
                    <button class="btn p-0" type="button" data-bs-toggle="dropdown">
                        <i class="ti ti-menu"></i>
                    ...
                    <div class="dropdown-menu dropdown-menu-end" style="position: fixed; z-index: 1050;">
                        <a class="dropdown-item bg-label-primary" href="{{ route('super-admin.users.show', user.user_id) }}">
                            <i class="ti ti-eye me-2"></i>
                            <span>Voir détails</span>
                        </a>
                        <a class="dropdown-item bg-label-primary" href="{{ route('super-admin.users.edit', user.user_id) }}">
                            <i class="ti ti-pencil-alt2 me-2"></i>
                            <span>Modifier</span>
                        </a>
                        <div class="dropdown-divider"></div>
                        ${user.is_active
                            ? `<a class="dropdown-item bg-label-warning" href="{{ route('super-admin.users.suspend', user.user_id) }}" onclick="return confirm('Êtes-vous sûr de vouloir suspendre cet utilisateur ?')">
                                   <i class="ti ti-control-pause me-2"></i>
                                   <span>Suspendre</span>
                               </a>`
                            : `<a class="dropdown-item bg-label-success" href="{{ route('super-admin.users.activate', user.user_id) }}" onclick="return confirm('Êtes-vous sûr de vouloir activer cet utilisateur ?')">
                                   <i class="ti ti-control-play me-2"></i>
                                   <span>Activer</span>
                               </a>`
                        }
                        <a class="dropdown-item bg-label-danger" href="{{ route('super-admin.users.delete', user.user_id) }}" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet utilisateur ? Cette action est irréversible.')">
                            <i class="ti ti-trash me-2"></i>
                            <span>Supprimer</span>
                        </a>
                    </div>
