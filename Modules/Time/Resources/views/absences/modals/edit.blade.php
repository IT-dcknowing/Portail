...
<div class="modal fade" id="editAbsenceModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form id="editAbsenceForm" action="" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="form-group col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control" id="edit_days" name="days" value="{{ $absence->days ?? '' }}" step="0.01" required>
                        </div>
...
<script>
    ...
    $(document).on('shown.bs.modal', '#editAbsenceModal', function() {
        // Calculer la différence de dates quand les champs de date changent
        $(document).on('change', '#edit_start_date, #edit_end_date', function() {
            calculerEditDifferenceDates();
        });
        
        // Mettre à jour les jours quand les heures changent
        $(document).on('input', '#edit_hours', function() {
            var hours = parseFloat(this.value) || 0;
            var workhours = parseFloat($('#edit_workhours').val()) || 8;
            var workdays = parseFloat($('#edit_workdays').val()) || 1;
            var taux = workhours / workdays;
            var days = hours / taux;
            $('#edit_days').val(days.toFixed(2));
        });
        ...
        $('#edit_days').on('input', function() {
            ...
            var taux = workhours / workdays;
            var hours = days * taux;
            $('#edit_hours').val(hours.toFixed(2));
        });
        ...
        $(document).on('click', '.remove-doc', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('company.times.absences.remove-document', ':id') }}".replace(':id', id),
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
                        success: function(response) {
                            if (response.success) {
                                $('#current_document').html('<div class="alert alert-success p-2">{{ __("Document supprimé avec succès.") }}</div>');
                                setTimeout(function() {
                                    $('#current_document').html('');
                                }, 2000);
                            } else {
                                toastr.error(response.message || "{{ __('Une erreur est survenue lors de la suppression du document.') }}");
                            }
                        },
                        error: function(xhr) {
                            console.error(xhr);
                            toastr.error("{{ __('Une erreur est survenue lors de la suppression du document.') }}");
                        }
                    });
                }
            });
